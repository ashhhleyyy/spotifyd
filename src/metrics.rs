...
use prometheus::{TextEncoder, Encoder, register_int_gauge, register_int_counter, IntGauge, IntCounter, opts};
...
pub async fn run_server(addr: SocketAddr) {
    let filter = warp::path("metrics")
        .map(|| {
            ...
            encoder.encode(&metrics, &mut buffer).expect("failed to encode");
